...
def output_images(ASes_output, originals):
    ...
    for image_nr in range(len(working_images)):  # loop through all
        ...
        cv2.imwrite(img_folder + "/" + "_out_" + image_names[image_nr],
...
def new_run(patch_used, original_images):
    ...
    for i in range(len(original_images)):
        tmp_result, tmp_adv_img = detector.new_detect_faces(original_images[i], patch_used, ground_truths[image_names[i]])
...
init_patch = tf.cast(init_patch, dtype = tf.int32) #let's go!

s2, a = new_run(init_patch, images)
output_images(s2, a)
